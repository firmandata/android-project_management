...
public class FileCachePersistent extends NetworkCachePersistent {
    ...
    public long setFileModelInfo(final FileModel fileModel) throws PersistenceError {
        long networkCacheId = 0;
        ...
        if (content != null) {
            try {
                ...
                // -- Save content to cache --
                networkCacheId = saveNetworkCacheContent(sqLiteDatabase, NetworkCachePersistentType.FILE_INFO, contentKey, content, 0);
            } catch (SQLException ex) {
                throw new PersistenceError(0, ex.getMessage(), ex);
            } catch (Exception ex) {
                throw new PersistenceError(0, ex.getMessage(), ex);
    ...
    public FileModel getFileModelInfo(final Integer fileId) throws PersistenceError {
        FileModel fileModel = null;
        ...
        try {
            ...
            // -- Get content from cache --
            content = getNetworkCacheContent(sqLiteDatabase, NetworkCachePersistentType.FILE_INFO, contentKey, 0);
        } catch (SQLException ex) {
            throw new PersistenceError(0, ex.getMessage(), ex);
        } catch (Exception ex) {
            throw new PersistenceError(0, ex.getMessage(), ex);
